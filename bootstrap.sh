...
if [ ! -f "$JAR_PATH" ]; then
    ...
    CHECKSUM_FILE="$MINIMA_DIR/minima.jar.sha256"
...
echo ""
echo "  On startup, the node will automatically:"
echo "    1. Connect to the default bootstrap peer (megammr.minima.global:9001)"
echo "    2. Download current peers from https://www.spartacusrex.com/minimapeers.txt"
echo "    3. Import them via RPC (peers action:addpeers) once the node is ready"
echo ""
echo "  To manually refresh peers later:"
echo "    PEERS=\$(curl -s https://www.spartacusrex.com/minimapeers.txt)"
echo "    ./minima/cli.sh \"peers action:addpeers peerslist:\$PEERS\""
echo ""
