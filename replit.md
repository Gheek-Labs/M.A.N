...
## Recent Changes
- 2026-02-21: Security audit and hardening: MDS password no longer logged to stdout (saved to .mds_password with 600 perms), Flask session secret auto-generated (no more hardcoded fallback), bootstrap.sh requires confirmation before sudo installs (--yes flag for CI), CSP + security headers added to Flask, XSS hardening (javascript: URI blocking, error escaping), command injection audit passed (SDK-only RPC, list-based subprocess), RPC port 9005 binding documented in network security table.
- 2026-02-21: Bootstrap auto-install: bootstrap.sh now auto-detects platform (apt/brew/dnf/pacman/apk) and installs missing Java, jq, curl automatically. Fixes "java not found" when cloned to non-Replit environments. start.sh also checks for Java with helpful error. Updated AGENT_START_HERE.md with prerequisites table and auto-install docs.
